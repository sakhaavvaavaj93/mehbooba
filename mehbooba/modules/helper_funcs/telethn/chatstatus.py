from mehbooba.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from mehbooba import DRAGONS
...
async def stenzle_is_admin(chat_id: int):
    status = False
    evil = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if stenzle.id == user.id:
            status = True
            break
    return status
